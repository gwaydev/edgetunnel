...
test('解析 schema v1 snapshot，并规范化 UUID 和用户映射', () => {
  const parsed = parseXboardSnapshot(snapshot(
...
test('schema v1 拒绝错误版本、非法 UUID、重复 UUID 和非法元数据', () => {
  assert.throws(() => parseXboardSnapshot(snapshot(undefined, undefined, { version: '1' })), /version/i);
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A, 'invalid'], { [UUID_A]: 1, invalid: 2 })), /uuid/i);
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A, UUID_A.toUpperCase()], { [UUID_A]: 1 })), /duplicate/i);
  assert.throws(() => parseXboardSnapshot(snapshot(undefined, undefined, { generatedAt: 'not-a-date' })), /generatedAt/i);
  assert.throws(() => parseXboardSnapshot(snapshot(undefined, undefined, { serverId: 0 })), /serverId/i);
  assert.throws(() => parseXboardSnapshot(snapshot(undefined, undefined, { serverId: '9' })), /serverId/i);
});

test('schema v1 要求 uuids 与 userMap 集合完全一致且用户 ID 为正整数', () => {
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A, UUID_B], { [UUID_A]: 1 })), /same UUID set/i);
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A], { [UUID_A]: 0 })), /user ID/i);
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A], { [UUID_A]: 1.5 })), /user ID/i);
  assert.throws(() => parseXboardSnapshot(snapshot([UUID_A], { [UUID_A]: 1, [UUID_B]: 2 })), /same UUID set/i);
});
...
test('仅 KV 读取异常可复用未超过 600 秒的旧快照，超过后 fail-closed', async () => {
  const first = await readXboardAccessContext({ XBOARD_KV: kv({ 'xboard:snapshot': snapshot() }) }, 10_000);
  assert.deepEqual([...first.uuids], [UUID_A]);

  const badStore = kv({}, new Error('KV unavailable'));
  const staleButAllowed = await readXboardAccessContext({ XBOARD_KV: badStore }, 10_000 + 590_000, true);
  ...
  const expired = await readXboardAccessContext({ XBOARD_KV: badStore }, 10_000 + 601_000, true);
...
test('KV 成功返回缺失或无效快照时立即 fail-closed，不复用旧缓存', async () => {
  await readXboardAccessContext({ XBOARD_KV: kv({ 'xboard:snapshot': snapshot() }) }, 10_000);

  for (const invalidValue of [null, '', '{', snapshot(undefined, undefined, { version: 2 })]) {
    const result = await readXboardAccessContext({ XBOARD_KV: kv({ 'xboard:snapshot': invalidValue }) }, 20_000, true);
    assertFailClosed(result);
  }

  const afterInvalidReadFailure = await readXboardAccessContext(
...
test('流量推送按 Xboard 用户聚合，非 2xx 时将批次合并回活动 Map', async () => {
  ...
  await assert.rejects(
    traffic.push(env, userMap, 50_000, async (url, init) => {
  ...
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [150, 225], [UUID_B]: [10, 20] });

  await traffic.push(env, userMap, 51_000, async (url, init) => {
    requests.push({ url, init });
  ...
  assert.deepEqual(traffic.snapshot(), { [UUID_B]: [10, 20] });
});
...
test('快照缺失时不会读取 legacy 双 key', async () => {
  ...
  const result = await readXboardAccessContext({ XBOARD_KV: store }, 4_000);
...
function createTrafficHarness() {
  let activeFetch = async () => { throw new Error('synthetic fetch implementation required'); };
  let activeCompletionClock = Date.now;
  const accumulator = createTrafficAccumulator({
    fetchImpl: (...args) => activeFetch(...args),
    log: () => {},
    completionClock: () => activeCompletionClock(),
  });

  return {
    add: accumulator.add,
    snapshot: accumulator.snapshot,
    push(env, userMap, now, fetchImpl = activeFetch, force = false, completionClock = Date.now) {
      activeFetch = fetchImpl;
      activeCompletionClock = completionClock;
...
test('连接关闭时的强制尾批次绕过常规推送间隔', async () => {
  ...
  traffic.add(UUID_A, 10, 20);
  await traffic.push(env, userMap, 1_000, send);
  traffic.add(UUID_A, 5, 7);

  assert.equal(await traffic.push(env, userMap, 2_000, send), false);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [5, 7] });
  assert.equal(await traffic.push(env, userMap, 2_000, send, true), true);
...
test('强制尾批次会等待在途推送并继续发送期间新增流量', async () => {
  ...
  traffic.add(UUID_A, 10, 20);
  const firstPush = traffic.push(env, userMap, 1_000, async (url, init) => {
    requests.push({ url, init });
    await firstResponse;
    return new Response('{}', { status: 200 });
  });
  ...
  traffic.add(UUID_A, 5, 7);
  const forcedFlush = traffic.push(env, userMap, 2_000, async (url, init) => {
    requests.push({ url, init });
    return new Response('{}', { status: 200 });
  }, true);
...
test('推送进行中产生的新流量保留在活动 Map，并可在下一批继续推送', async () => {
  ...
  traffic.add(UUID_A, 10, 20);
  const firstPush = traffic.push(env, userMap, 60_000, async (url, init) => {
    requests.push({ url, init });
    await firstResponse;
    return new Response('{}', { status: 200 });
  });
  ...
  await Promise.resolve();
  traffic.add(UUID_A, 5, 7);
  const inFlightPush = traffic.push(env, userMap, 60_001, async () => {
  ...
  await traffic.push(env, userMap, 61_000, async (url, init) => {
    requests.push({ url, init });
    return new Response('{}', { status: 200 });
  });
...
test('默认 900 秒 TTL：孤儿流量在边界前保留且不 fetch，在边界丢弃', async () => {
  ...
  traffic.add(UUID_A, 100, 200, 0);
  assert.equal(await traffic.push(trafficEnv(), {}, 899_999, noFetch, true), false);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [100, 200] });

  assert.equal(await traffic.push(trafficEnv(), {}, 900_000, noFetch, true), false);
  assert.deepEqual(traffic.snapshot(), {});
...
test('孤儿流量超过默认 TTL 时丢弃且不 fetch', async () => {
  ...
  assert.equal(await traffic.push(trafficEnv(), {}, 900_001, async () => {
    fetchCalls++;
    throw new Error('orphan traffic must not fetch');
  }, true), false);
...
test('有限有效 TTL 最小 clamp 为 60 秒', async () => {
  ...
  traffic.add(UUID_A, 1, 2, 0);
  await traffic.push(env, {}, 59_999, noFetch, true);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [1, 2] });

  await traffic.push(env, {}, 60_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), {});
});
...
test('缺失或无效 TTL 安全回退到默认 900 秒', async () => {
  for (const invalidValue of [undefined, '', 'not-a-number', '0', '-5', 'Infinity']) {
    ...
    traffic.add(UUID_A, 1, 2, 0);
    await traffic.push(env, {}, 899_999, noFetch, true);
    assert.deepEqual(traffic.snapshot(), { [UUID_A]: [1, 2] }, `value ${String(invalidValue)} must retain before 900 seconds`);

    await traffic.push(env, {}, 900_000, noFetch, true);
    assert.deepEqual(traffic.snapshot(), {}, `value ${String(invalidValue)} must expire at 900 seconds`);
  }
});
...
test('缺 credentials 时仍清理到期 orphan，mapped traffic 保留且不 fetch', async () => {
  ...
  traffic.add(UUID_B, 30, 40, 0);

  assert.equal(await traffic.push({
  ...
  assert.deepEqual(traffic.snapshot(), { [UUID_B]: [30, 40] });
  assert.equal(fetchCalls, 0);
});
...
test('interval gate 时仍清理到期 orphan，mapped traffic 保留且不额外 fetch', async () => {
  ...
  traffic.add(UUID_A, 1, 2, 0);
  await traffic.push(env, { [UUID_A]: 42 }, 1_000, async () => new Response('{}', { status: 200 }));

  traffic.add(UUID_A, 10, 20, 0);
  traffic.add(UUID_B, 30, 40, 0);
  assert.equal(await traffic.push(env, { [UUID_B]: 42 }, 60_000, async () => {
    gatedFetchCalls++;
...
test('backoff gate 时仍清理到期 orphan，mapped traffic 保留且不额外 fetch', async () => {
  ...
  traffic.add(UUID_A, 10, 20, 60_000);

  await assert.rejects(traffic.push(env, { [UUID_A]: 42 }, 60_000, async () => (
    new Response('synthetic failure', { status: 503 })
  ), false, () => 60_000), /503/);

  traffic.add(UUID_B, 30, 40, 0);
  let gatedFetchCalls = 0;
  assert.equal(await traffic.push(env, { [UUID_A]: 42 }, 60_500, async () => {
    gatedFetchCalls++;
    throw new Error('backoff gate must not fetch');
  }), false);

  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [10, 20] });
  assert.equal(gatedFetchCalls, 0);
});
...
test('当前 userMap 中的有效流量仍按用户聚合发送，成功后缓存清空', async () => {
  ...
  const requests = [];
  traffic.add(UUID_A, 100, 200, 0);
  traffic.add(UUID_B, 10, 20, 10);

  assert.equal(await traffic.push(trafficEnv(), { [UUID_A]: 42, [UUID_B]: 42 }, 1_000, async (url, init) => {
    requests.push({ url, init });
    return new Response('{}', { status: 200 });
  }, true), true);
  ...
  traffic.add(UUID_A, 1, 2, 0);
  await traffic.push(trafficEnv(), {}, 900_000, async () => {
    throw new Error('orphan traffic must not fetch');
  }, true);
  assert.deepEqual(traffic.snapshot(), {}, 'successful batches must delete stale timestamp metadata');
});
...
test('长 in-flight 的合法 batch 非 2xx 后从真正失败时刻刷新 TTL', async () => {
  ...
  const push = traffic.push(env, { [UUID_A]: 42 }, 10_000, async () => {
    await fetchGate;
  ...
  const noFetch = async () => { throw new Error('orphan traffic must not fetch'); };
  await traffic.push(env, {}, 120_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [10, 20] });

  await traffic.push(env, {}, 179_999, noFetch, true);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [10, 20] });

  await traffic.push(env, {}, 180_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), {});
});
...
test('合法 batch 抛错后合并回缓存', async () => {
  ...
  await assert.rejects(traffic.push(trafficEnv(), { [UUID_A]: 42 }, 1_000, async () => {
...
test('失败回并不丢并发新增流量，且不会倒退更晚的 timestamp', async () => {
  ...
  await Promise.resolve();
  completionNow = 120_000;
  traffic.add(UUID_A, 5, 7, 130_000);
  releaseFetch();

  await assert.rejects(push, /503/);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [15, 27] });

  const noFetch = async () => { throw new Error('orphan traffic must not fetch'); };
  await traffic.push(env, {}, 189_999, noFetch, true);
  assert.deepEqual(traffic.snapshot(), { [UUID_A]: [15, 27] });

  await traffic.push(env, {}, 190_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), {});
});

test('reset 清空流量状态，公开快照只暴露 synthetic aggregate', async () => {
  let traffic = createTrafficHarness();
  traffic.add(UUID_A, 10, 20, 123_456);
  const snapshotValue = traffic.snapshot();
  ...
  assert.deepEqual(traffic.snapshot(), {}, 'reset must delete stale timestamp metadata');
});

test('孤儿过期清理同时删除 timestamp 元数据', async () => {
  const traffic = createTrafficHarness();
  const noFetch = async () => { throw new Error('orphan traffic must not fetch'); };
  traffic.add(UUID_A, 10, 20, 1_000_000);
  await traffic.push(trafficEnv(), {}, 1_900_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), {});

  traffic.add(UUID_A, 1, 2, 0);
  await traffic.push(trafficEnv(), {}, 900_000, noFetch, true);
  assert.deepEqual(traffic.snapshot(), {}, 'expired orphan cleanup must delete stale timestamp metadata');
});

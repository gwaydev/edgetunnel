...
function 是Xboard强制KV模式(env = {}) {
	const value = env.XBOARD_KV_REQUIRED;
...
function 创建Xboard流量记录器(uuidValue, accessContext, env, ctx, accumulator = Xboard上报累加器, request = null, timerApi = globalThis) {
	...
	const clientIp = readXboardClientIp(request);
	const configuredInterval = env?.XBOARD_ONLINE_PUSH_INTERVAL_SECONDS;
	const parsedInterval = configuredInterval === undefined ? 60 : Number(configuredInterval);
	const heartbeatIntervalMs = Number.isFinite(parsedInterval) && parsedInterval > 0
		? Math.min(240, Math.max(1, parsedInterval)) * 1000
		: 0;
	const setTimer = typeof timerApi?.setTimeout === 'function' ? timerApi.setTimeout.bind(timerApi) : null;
	const clearTimer = typeof timerApi?.clearTimeout === 'function' ? timerApi.clearTimeout.bind(timerApi) : null;
...
export default {
	async fetch(request, env, ctx) {
		// A fresh Xboard deployment uses one explicit KV binding. Reuse it for
		// the Worker's non-sensitive runtime configuration keys when the legacy
		// `KV` alias is not configured. Snapshot authorization still reads the
		// fixed `XBOARD_KV` binding directly.
		if (!env.KV && env.XBOARD_KV) env = Object.assign(Object.create(env), { KV: env.XBOARD_KV });
		...
		const Xboard强制KV模式 = 是Xboard强制KV模式(env);
		const 允许代理请求 = Boolean(管理员密码) || Xboard强制KV模式;
		...
		if (访问路径 === 'version') {// 版本信息接口
		...
		} else if (允许代理请求 && upgradeHeader === 'websocket') {// WebSocket代理
		...
		} else if (允许代理请求 && !访问路径.startsWith('admin/') && 访问路径 !== 'login' && !是订阅路径 && request.method === 'POST') {// gRPC/XHTTP代理
			const 反代上下文 = await 反代参数获取(url, userID, 默认反代IP, 默认反代兜底);
			...
			// Xboard service-to-service subscription requests use the explicit sync
			// token and XBOARD_KV authorization model, not the legacy personal
			// ADMIN/UUID mode. Evaluate this before the old login gate so a fresh
			// production Worker can serve adaptive subscriptions without ADMIN.
			const Xboard服务端JSON请求 = 是订阅路径 && 是Xboard服务端JSON订阅请求(request, env);
			const Xboard服务端请求订阅 = Xboard服务端JSON请求 && await 校验Xboard订阅鉴权(request, env);
			if (Xboard服务端JSON请求 && !Xboard服务端请求订阅) return 创建Xboard订阅鉴权拒绝响应();
			if (!管理员密码 && !Xboard服务端请求订阅) return fetch(Pages静态页面 + '/noADMIN').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }) });
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					// Service-to-service authorization was evaluated before the legacy
					// personal-mode gate above. Keep the common subscription renderer
					// focused on content generation here.
					...
					if (用户客户端请求订阅 || Xboard服务端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						config_JSON = await 读取config_JSON(env, host, userID, UA);
